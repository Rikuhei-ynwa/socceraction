...
def convert_to_actions(
    ...
    actions.loc[events.type_name == 'Shot', ['start_x', 'start_y']] = _convert_ball_locations(
    ...
    actions.loc[events.type_name == 'Shot', ['end_x', 'end_y']] = _convert_ball_locations(
...
def convert_to_actions_360(
    ...
    actions[['type_id', 'result_id', 'bodypart_id']] = (
        events[['type_name', 'extra']].apply(
            _parse_event, axis=1, result_type='expand'
            ))
    actions["away_team"] = (
...
def _convert_x_cie(
    ...
    return x_cie / spadlconfig.FIELD_LENGTH_STATSBOMB * spadlconfig.FIELD_LENGTH
...
def _convert_y_cie(
    ...
    return spadlconfig.FIELD_WIDTH - (
        y_cie / spadlconfig.FIELD_WIDTH_STATSBOMB * spadlconfig.FIELD_WIDTH
        )


def _convert_ball_locations(locations: pd.Series, fidelity_version: int) -> npt.NDArray[np.float32]:
    ...
    for i, loc in enumerate(locations):
        if isinstance(loc, list) and len(loc) == 2:
            coordinates[i, 0] = _convert_x_cie(loc[0], cell_relative_center)
            coordinates[i, 1] = _convert_y_cie(loc[1], cell_relative_center)
            ...
            CELL_SHOT_SIDE = 0.05
            coordinates[i, 0] = _convert_x_cie(loc[0], cell_relative_center)
            coordinates[i, 1] = _convert_y_cie(loc[1], CELL_SHOT_SIDE)
...
def _convert_player_locations(
    ...
    cell_side = 0.1 if fidelity_version == 2 else 1.0
    cell_relative_center = cell_side / 2
    location[:, 0] = _convert_x_cie(location[:, 0], cell_relative_center)
    location[:, 1] = _convert_y_cie(location[:, 1], cell_relative_center)
    return location
...
def _parse_foul_event(extra: dict[str, Any]) -> tuple[str, str, str]:
    ...
    r = 'fail' # default
    if extra.get('foul_committed', {}).get('offensive', {}) == True:
        a = 'offensive_foul'
    else:
        a = 'defensive_foul'
